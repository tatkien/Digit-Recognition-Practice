...
def output_layer_activation_derivative(x):
    ...
    return 1
rectified_linear_unit = np.vectorize(rectified_linear_unit)
output_layer_activation = np.vectorize(output_layer_activation)
...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        hidden_layer_error = np.multiply(np.dot(self.hidden_to_output_weights.T, output_layer_error), rectified_linear_unit_derivative(hidden_layer_weighted_input))  # (3, 1)

        bias_gradients = hidden_layer_error
        hidden_to_output_weight_gradients = np.dot(output_layer_error, hidden_layer_activation.T)  # (1, 3)
        input_to_hidden_weight_gradients = np.dot(hidden_layer_error, input_values.T)  # (3, 2)
