...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    # YOUR CODE HERE
    train_x, train_y, test_x, test_y = get_MNIST_data()
    train_x, test_x = np.mod(train_x, 3), np.mod(test_x, 3)
...
# print('softmax test error with 18 - dimensional PCA',compute_test_error(test_pca, test_y, theta, temp_parameter=1))
...
# print('softmax test error with cubic kernel',compute_test_error(test_cube, test_y, theta, temp_parameter=1))

# Find error rate for 10-dimensional PCA feature mapping with cubic kernel
# using SVM in sklearn library
...
# print('SVM test error with cubic kernel', 1 - np.mean(pred_test_y == test_y))

# # RBF SVM Using sklearn
# svm_model = SVC(random_state=0,kernel='rbf')
# svm_model.fit(train_pca10, train_y)
# pred_test_y = svm_model.predict(test_pca10)
# print('RBF SVM test error with cubic kernel',1-np.mean(pred_test_y == test_y))

# Train the softmax regression model using the kernelized features
train_polynomial_kernelized = polynomial_kernel(train_x, train_x, c=1, p=3)
test_polynomial_kernelized = polynomial_kernel(test_x, train_x, c=1, p=3)
train_rbf_kernelized = rbf_kernel(train_x, train_x, gamma=0.05)
test_rbf_kernelized = rbf_kernel(test_x, train_x, gamma=0.05)

theta, _ = softmax_regression(train_polynomial_kernelized, train_y, temp_parameter=1, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
print('Polynomial Kernel test error',compute_test_error(test_polynomial_kernelized, test_y, theta, temp_parameter=1))
theta, _ = softmax_regression(train_rbf_kernelized, train_y, temp_parameter=1, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
print('RBF Kernel test error',compute_test_error(test_rbf_kernelized, test_y, theta, temp_parameter=1))